    ...
    <div class="md:grid grid-cols-2 gap-4">
        ...
        <div class="flex flex-col">
            <div class="flex items-center">
                ...
                <input type="checkbox" v-model="isCurrentJob" :id="'emprego_atual' + index" class="h-4 w-4"/>
                <label :for="'emprego_atual' + index" class="font-medium">Emprego Atual</label>
            </div>
            <input type="date" v-model="experience.data_fim" :disabled="isCurrentJob"
                   class="mt-1 p-2 border rounded focus:outline-none focus:ring focus:border-blue-300"/>
        </div>
        <div class="flex flex-col">
            <label>Tipo</label>
...
const isCurrentJob = computed({
    get() {
        return Boolean(props.experience.emprego_atual);
    },
    set(value) {
        props.experience.emprego_atual = value ? 1 : 0;
    }
});
</script>
